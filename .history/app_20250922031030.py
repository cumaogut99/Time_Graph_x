...
class TimeGraphApp(QMainWindow):
    ...
    def _load_data_with_settings(self, settings: dict):
        ...
        try:
            ...
            if active_container and hasattr(active_container.plot_manager, 'enable_datetime_axis'):
                if hasattr(self, '_datetime_converted') and self._datetime_converted:
                    # Enable datetime formatting for better readability
                    ...
                    # Also enable datetime formatting in statistics panel
                    if hasattr(self.time_graph_widget, 'statistics_panel') and hasattr(self.time_graph_widget.statistics_panel, 'set_datetime_axis'):
                        self.time_graph_widget.statistics_panel.set_datetime_axis(True)
                        logger.info("Datetime formatting enabled in statistics panel")
                else:
                    # Disable datetime formatting for normal numeric data
                    active_container.plot_manager.enable_datetime_axis(False)
                    logger.info("Datetime axis formatting disabled for numeric data")
                    
                    # Also disable datetime formatting in statistics panel
                    if hasattr(self.time_graph_widget, 'statistics_panel') and hasattr(self.time_graph_widget.statistics_panel, 'set_datetime_axis'):
                        self.time_graph_widget.statistics_panel.set_datetime_axis(False)
                        logger.info("Datetime formatting disabled in statistics panel")
            ...
            # Finish loading operation on error
            self.loading_manager.finish_operation("file_loading")
            ...
            if time_column_name and time_column_name in df.get_column_names():
                ...
                if converted_values is not None:
                    ...
                    if 'datetime64' in str(converted_values.dtype):
                        ...
                        # Mark that we converted datetime for later axis formatting
    ...
    def _load_data_file(self, file_path: str):
        ...
        try:
            ...
            if file_ext == '.csv':
                ...
                # Excel dosyaları için pandas üzerinden yükleme
    ...
    def _save_data_file(self, file_path: str, file_filter: str):
        ...
        try:
            ...
            elif file_ext == '.xlsx' or 'Excel' in file_filter:
                # Excel formatında kaydetme için pandas kullan
                df_pandas = df.to_pandas_df()
                df_pandas.to_excel(file_path, index=False)
            else:
                # Varsayılan CSV
                df.export_csv(file_path)
