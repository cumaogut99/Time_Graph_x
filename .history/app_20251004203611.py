...
class DataLoader(QObject):
    ...
    def _sanitize_dataframe(self, df):
        ...
        try:
            ...
            logger.info(f"✅ Veri temizleme tamamlandı: {df.height} satır, {len(df.columns)} kolon")
...
class TimeGraphApp(QMainWindow):
    ...
    def _show_data_quality_summary(self, df, filename=""):
        ...
        try:
            ...
            # Log raporu
            logger.info(f"📊 Veri Kalite Raporu - {filename}")
            logger.info(f"   ✅ Toplam: {total_rows} satır, {total_cols} kolon")
            
            if high_null_cols:
                logger.info(f"   ⚠️ Yüksek NULL oranı olan kolonlar: {len(high_null_cols)}")
                for col, pct, count in high_null_cols[:5]:
                    logger.info(f"      • '{col}': {count} NULL (%{pct:.1f}) - otomatik düzeltildi")
            else:
                logger.info(f"   ✅ Tüm kolonlar temiz (düşük NULL oranı)")
                
            logger.info(f"   📈 Veri kullanıma hazır!")
