...
class TimeGraphApp(QMainWindow):
    ...
    def _on_file_switched(self, new_index: int, old_index: int):
        ...
        if old_index >= 0 and old_index != new_index:
            ...
            logger.debug(f"Saved state for previous file (index: {old_index})")
        
        logger.info(f"Switching to file: {file_data['filename']} (index: {new_index})")
    ...
    def _on_data_processing_complete(self):
        ...
        try:
            ...
            if saved_state is None:
                logger.debug(f"First time loading file {self._pending_restore_index}, saving initial widget state")
