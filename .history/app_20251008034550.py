...
class TimeGraphApp(QMainWindow):
    ...
    def __init__(self):
        ...
        # Threading members - FİX: Her dosya için ayrı thread yerine tek thread pool
    ...
    def _setup_ui(self):
        ...
        # Ana widget'ı oluştur (dosya sekmeleri status bar'da olacak)
    ...
    def on_loading_finished(self, df, time_column):
        ...
        try:
            ...
            file_metadata = {
                ...
                'widget_state': None,  # Şimdilik boş
                'is_data_modified': False
            }
            
            # FİX: file_manager.add_file() zaten file_switched sinyali gönderir
            # O da _on_file_switched'i çağırır ve update_data yapar
            # Bu yüzden burada update_data çağırmaya gerek yok!
            ...
            # NOT: Widget'a data yükleme _on_file_switched callback'inde yapılıyor
            # Burada sadece datetime axis ayarını yapalım
    ...
    def _save_widget_state_for_file(self, file_index: int):
        """Belirtilen dosya için widget durumunu kaydet."""
        if file_index < 0 or not self.time_graph_widget:
            return
        
        try:
            # Widget'tan signal mapping ve graph settings'i al
            widget_state = {
                'graph_signal_mapping': {},
                'graph_settings': {},
                'current_tab_index': 0,
                'tab_count': 0,
                'tab_names': []
            ...
            # Graph signal mapping'i kaydet
            if hasattr(self.time_graph_widget, 'graph_signal_mapping'):
                widget_state['graph_signal_mapping'] = self.time_graph_widget.graph_signal_mapping.copy()
            
            # Graph settings'i kaydet
            if hasattr(self.time_graph_widget, 'graph_settings'):
                widget_state['graph_settings'] = self.time_graph_widget.graph_settings.copy()
            
            # Grafik sekmelerini kaydet
            if hasattr(self.time_graph_widget, 'tab_widget') and self.time_graph_widget.tab_widget:
                tab_widget = self.time_graph_widget.tab_widget
                widget_state['current_tab_index'] = tab_widget.currentIndex()
                widget_state['tab_count'] = tab_widget.count()
                
                # Her sekmenin adını kaydet
                widget_state['tab_names'] = []
                for i in range(tab_widget.count()):
                    tab_name = tab_widget.tabText(i)
                    widget_state['tab_names'].append(tab_name)
            
            # State'i file manager'a kaydet
            self.file_manager.save_widget_state(file_index, widget_state)
            
            logger.info(f"[STATE SAVE] Widget state saved for file {file_index}: "
                        f"{widget_state['tab_count']} tabs, "
                        f"tab names: {widget_state.get('tab_names', [])}, "
                        f"{len(widget_state.get('graph_signal_mapping', {}))} graph mappings, "
                        f"current tab: {widget_state['current_tab_index']}")
            logger.info(f"[STATE SAVE] Graph signal mapping keys: {list(widget_state.get('graph_signal_mapping', {}).keys())}")
            
        except Exception as e:
            logger.warning(f"Failed to save widget state for file {file_index}: {e}")
    
    def _restore_widget_state_for_file(self, file_index: int):
        """Belirtilen dosya için widget durumunu geri yükle."""
        if file_index < 0 or not self.time_graph_widget:
            return
        
        try:
            widget_state = self.file_manager.get_widget_state(file_index)
            
            if widget_state:
                # Önce mevcut grafik sekmelerini temizle ve yenilerini oluştur
                if 'tab_count' in widget_state:
                    current_tab_count = self.time_graph_widget.tab_widget.count() if self.time_graph_widget.tab_widget else 0
                    target_tab_count = widget_state['tab_count']
                    tab_names = widget_state.get('tab_names', [])
                    
                    logger.info(f"[STATE RESTORE] Adjusting tabs from {current_tab_count} to {target_tab_count}")
                    logger.info(f"[STATE RESTORE] Target tab names: {tab_names}")
                    
                    # Tüm mevcut sekmeleri temizle
                    if self.time_graph_widget.tab_widget and current_tab_count > 0:
                        logger.info(f"[STATE RESTORE] Removing {current_tab_count} existing tabs")
                        # Geriye doğru sil (index kayması olmasın)
                        for i in range(current_tab_count - 1, -1, -1):
                            widget_to_remove = self.time_graph_widget.tab_widget.widget(i)
                            self.time_graph_widget.tab_widget.removeTab(i)
                            if widget_to_remove in self.time_graph_widget.graph_containers:
                                self.time_graph_widget.graph_containers.remove(widget_to_remove)
                            # Widget'ı temizle
                            if widget_to_remove:
                                widget_to_remove.deleteLater()
                    
                    # graph_containers listesini de temizle
                    self.time_graph_widget.graph_containers.clear()
                    logger.info(f"[STATE RESTORE] Cleared all existing tabs and containers")
                    
                    # Yeni sekmeleri oluştur
                    logger.info(f"[STATE RESTORE] Creating {target_tab_count} new tabs")
                    for i in range(target_tab_count):
                        tab_name = tab_names[i] if i < len(tab_names) else f"Graf {i+1}"
                        self.time_graph_widget._add_tab(tab_name)
                        logger.info(f"[STATE RESTORE] Created tab {i}: '{tab_name}'")
                
                # Graph signal mapping'i restore et
                if 'graph_signal_mapping' in widget_state:
                    ...
                    logger.debug(f"Restored graph signal mapping with {len(widget_state['graph_signal_mapping'])} tab mappings")
                
                # Graph settings'i restore et
                if 'graph_settings' in widget_state:
                    self.time_graph_widget.graph_settings = widget_state['graph_settings'].copy()
                
                # Tab index'i restore et
                if 'current_tab_index' in widget_state and hasattr(self.time_graph_widget, 'tab_widget'):
                    tab_index = widget_state['current_tab_index']
                    if self.time_graph_widget.tab_widget and tab_index < self.time_graph_widget.tab_widget.count():
                        self.time_graph_widget.tab_widget.setCurrentIndex(tab_index)
                        logger.debug(f"Restored tab index to {tab_index}")
                
                # Grafikleri yeniden çiz
                self.time_graph_widget._redraw_all_signals()
                
                logger.info(f"[STATE RESTORE] Widget state restored for file {file_index}: {widget_state.get('tab_count', 0)} tabs")
                logger.info(f"[STATE RESTORE] Final container count: {len(self.time_graph_widget.graph_containers)}")
            else:
                logger.info(f"[STATE RESTORE] No saved widget state for file {file_index}, starting clean")
                
        except Exception as e:
            logger.warning(f"Failed to restore widget state for file {file_index}: {e}")
    ...
    def _on_file_switched(self, new_index: int, old_index: int):
        ...
        # Önceki dosyanın state'ini kaydet
        if old_index >= 0 and old_index != new_index:
            logger.info(f"[FILE SWITCH] Saving state for previous file (index: {old_index})")
            self._save_widget_state_for_file(old_index)
        
        logger.info(f"[FILE SWITCH] Switching from file {old_index} to file {new_index}: {file_data['filename']}")
        
        # Restore işlemini processing bittikten sonra yapmak için index'i sakla
        self._pending_restore_index = new_index
        
        try:
            # Processing thread'in finished sinyalini dinle
            # NOT: update_data her çağrıldığında yeni bir thread oluşturuyor
            # Bu yüzden bağlantıyı update_data'dan ÖNCE yapamayız
            # Bunun yerine processing_worker.finished sinyalini yakalayacağız
            
            # Data'yı widget'a yükle (bu async işlem başlatır)
            logger.debug(f"Loading data to widget: {file_data['df'].height} rows")
            self.time_graph_widget.update_data(
                file_data['df'], 
                time_column=file_data['time_column']
            )
            
            # Processing thread'in finished sinyalini yakala
            if hasattr(self.time_graph_widget, 'processing_thread') and self.time_graph_widget.processing_thread:
                # Önce eski bağlantıları temizle (varsa)
                try:
                    self.time_graph_widget.processing_thread.finished.disconnect(self._on_data_processing_complete)
                except:
                    pass
                # Yeni bağlantı kur
                self.time_graph_widget.processing_thread.finished.connect(self._on_data_processing_complete)
                logger.debug("Connected to processing thread finished signal")
            
            logger.debug("Widget data update started (async)")
    ...
    def _on_data_processing_complete(self):
        """Data processing tamamlandığında çağrılır - widget state'i restore et."""
        if not hasattr(self, '_pending_restore_index') or self._pending_restore_index < 0:
            return
        
        file_data = self.file_manager.get_file_data(self._pending_restore_index)
        if not file_data:
            return
        
        try:
            logger.debug(f"Data processing complete, restoring state for file {self._pending_restore_index}")
            ...
            # Eğer bu dosya için kaydedilmiş widget state yoksa (ilk yükleme), mevcut durumu kaydet
            saved_state = self.file_manager.get_widget_state(self._pending_restore_index)
            if saved_state is None:
                logger.info(f"[DATA COMPLETE] First time loading file {self._pending_restore_index}, saving initial widget state")
                self._save_widget_state_for_file(self._pending_restore_index)
            else:
                logger.info(f"[DATA COMPLETE] Restoring saved state for file {self._pending_restore_index}")
                # Widget state'i restore et (ŞİMDİ güvenli - processing bitti)
                self._restore_widget_state_for_file(self._pending_restore_index)
