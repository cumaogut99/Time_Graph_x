...
logging.basicConfig(
    level=logging.WARNING,  # INFO logları devre dışı
...
class OldDataLoader(QObject):
    ...
    def _load_data(self):
        ...
        csv_opts = {
            ...
            'try_parse_dates': True,  # ✅ FIX: Enable datetime parsing (consistent with cache)
        ...
        if file_ext == '.csv':
            try:
                ...
                # Özel delimiter kullanılıyorsa cache bypass edilmeli
                ...
                if df is None:
                    # Cache başarısız veya özel delimiter, normal CSV okuma
                    logger.warning("Cache not used or failed, reading CSV directly with custom settings")
        ...
        try:
            ...
            if time_column_name and time_column_name in df.columns:
                ...
                if time_format == 'Unix Timestamp':
                    ...
                    elif original_series.dtype == pl.Datetime:
                        logger.info(f"Zaman kolonu '{time_column_name}' zaten Datetime tipinde (Polars tarafından parse edilmiş)")
                        converted_series = original_series
                    else:
                        # String datetime - birden fazla format dene
                        logger.info(f"Zaman kolonu '{time_column_name}' zaman formatına dönüştürülüyor.")
                        
                        # Yaygın datetime formatları
                        datetime_formats = [
                            None,  # Otomatik algılama (Polars default)
                        ...
                        for fmt in datetime_formats:
                            try:
                                if fmt is None:
                                    ...
                                    converted_series = original_series.str.strptime(pl.Datetime, format=fmt, strict=False)
                                
                                # Dönüşüm başarılı mı kontrol et (tümü null değil mi?)
                                if converted_series is not None and converted_series.null_count() < len(converted_series):
                                    logger.info(f"Datetime parsing başarılı, format: {fmt or 'otomatik'}")
                                    break
                            except Exception as e:
                                continue
                        
                        # Tüm formatlar başarısız olduysa, sayısal olarak dene
                        if converted_series is None or converted_series.null_count() == len(converted_series):
...
class TimeGraphApp(QMainWindow):
    ...
    def _on_project_save(self):
        ...
        try:
            ...
            if self.loading_manager:
                self.loading_manager.start_operation("project_save", "Proje kaydediliyor...")
            ...
            if self.loading_manager:
                self.loading_manager.finish_operation("project_save")
    ...
    def _on_project_open(self):
        ...
        try:
            ...
            # Show loading overlay
            if self.loading_manager:
                self.loading_manager.start_operation("project_load", "Proje yükleniyor...")
            ...
            if project_data is None:
                if self.loading_manager:
                    self.loading_manager.finish_operation("project_load")
                QMessageBox.critical(
                    self,
            ...
            if existing_index >= 0:
                ...
                if self.file_manager.file_tab_widget:
                    self.file_manager.file_tab_widget.setCurrentIndex(existing_index)
                if self.loading_manager:
                    self.loading_manager.finish_operation("project_load")
            ...
            # Add file metadata first (this will get the file_index)
            file_metadata = {
                'file_path': file_path,
                'filename': original_filename,
                'df': dataframe,
                'time_column': time_column,
                'widget': None,  # Will be created below
                'is_data_modified': False,
            ...
            # Load data into widget (dataframe is already polars)
            new_widget.update_data(dataframe, time_column)
            
            # Apply layout configuration after a short delay
