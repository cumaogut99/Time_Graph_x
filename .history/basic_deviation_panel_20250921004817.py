...
class BasicDeviationPanel(QWidget):
    ...
    def _setup_ui(self):
        ...
        # Parameter Selection Group - Make it more prominent
        ...
        # Parameter selection with better styling
        self.parameter_combo = QComboBox()
        self.parameter_combo.addItem("All Parameters")
        
        # Debug: Add some test items if no signals available
        if not self.all_signals:
            self.parameter_combo.addItem("No signals available")
            self.parameter_combo.addItem("Test Signal 1")
            self.parameter_combo.addItem("Test Signal 2")
            logger.warning("No signals provided to BasicDeviationPanel")
        else:
            for signal in self.all_signals:
                self.parameter_combo.addItem(signal)
            logger.info(f"Added {len(self.all_signals)} signals to parameter combo")
            
        self.parameter_combo.setStyleSheet(self._get_combo_style())
        self.parameter_combo.setMinimumHeight(35)  # Make it taller
        
        # Add label with better styling
        param_label = QLabel("Select Parameter:")
        param_label.setStyleSheet("color: #e6f3ff; font-weight: bold; font-size: 13px;")
        parameter_layout.addRow(param_label, self.parameter_combo)
