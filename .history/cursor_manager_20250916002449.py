...
class CursorManager(QObject):
    ...
    def _handle_dual_cursor_click(self, x_pos: float):
        ...
        if not self.dual_cursors_1:
            self.dual_cursors_1 = []
            self._create_dual_cursor_set(self.dual_cursors_1, x_pos, 'red', self._sync_dual_cursors_1)
            logger.debug(f"First dual cursors created at position: {x_pos:.3f}")
            self._emit_cursor_positions()
            return
        ...
        if not self.dual_cursors_2:
            self.dual_cursors_2 = []
            self._create_dual_cursor_set(self.dual_cursors_2, x_pos, 'blue', self._sync_dual_cursors_2)
            logger.debug(f"Second dual cursors created at position: {x_pos:.3f}")
            self._emit_cursor_positions()
            return
    ...
    def zoom_to_cursors(self):
        """Zoom to the region between dual cursors."""
        if self.current_mode != "dual" or not (self.dual_cursors_1 and self.dual_cursors_2):
            logger.warning("Zoom to cursors requires dual cursor mode with both cursors active")
            return False
        ...
        try:
            ...
            for plot_widget in self.plot_widgets:
                try:
                    # Set X-axis range
                    plot_widget.setXRange(zoom_start, zoom_end, padding=0)
                    # Keep Y-axis auto-scaled
                    plot_widget.enableAutoRange(axis='y')
                except Exception as e:
                    logger.error(f"Error zooming plot widget: {e}")
                    
            return True
            
        except Exception as e:
            logger.error(f"Error in zoom_to_cursors: {e}")
            return False
