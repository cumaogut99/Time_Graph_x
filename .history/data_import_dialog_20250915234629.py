...
from PyQt5.QtWidgets import (
    ...
    QProgressBar, QMessageBox, QFrame, QScrollArea, QGridLayout, QLineEdit
)
...
class DataImportDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        # Ana splitter (sol: önizleme, sağ: ayarlar)
    ...
    def _create_info_panel(self):
        """Üst bilgi paneli oluştur."""
        frame = QFrame()
        frame.setFrameStyle(QFrame.StyledPanel)
        layout = QHBoxLayout(frame)
        
        # Dosya bilgisi
        self.file_info_label = QLabel(f"📁 Dosya: {os.path.basename(self.file_path)}")
        self.file_info_label.setFont(QFont("Arial", 10, QFont.Bold))
        ...
        try:
            ...
            self.size_label = QLabel(f"📊 Boyut: {size_mb:.1f} MB")
    ...
    def _create_preview_panel(self):
        ...
        # Metadata bilgisi
        self.metadata_label = QLabel("Veri yükleniyor...")
    ...
    def _create_column_settings(self):
        ...
        group = QGroupBox("📊 Sütun Ayarları")
        layout = QGridLayout(group)
        
        # Header satırı
        layout.addWidget(QLabel("Header Satırı:"), 0, 0)
    ...
    def _create_row_settings(self):
        ...
        layout.addWidget(QLabel("Atlanacak Satır:"), 1, 0)
        self.skip_rows_spinbox = QSpinBox()
        self.skip_rows_spinbox.setRange(0, 100)
        self.skip_rows_spinbox.setValue(0)
        layout.addWidget(self.skip_rows_spinbox, 1, 1)
    ...
    def _create_time_column_settings(self):
        ...
        # Zaman kolonu seçimi
        layout.addWidget(QLabel("Zaman Kolonu:"), 0, 0)
        self.time_column_combo = QComboBox()
        self.time_column_combo.addItem("Otomatik Algıla")
        layout.addWidget(self.time_column_combo, 0, 1)
        
        # Zaman formatı
        layout.addWidget(QLabel("Zaman Formatı:"), 1, 0)
        self.time_format_combo = QComboBox()
        self.time_format_combo.addItems([
            'Otomatik', '%Y-%m-%d %H:%M:%S', '%d/%m/%Y %H:%M:%S', 
            '%Y-%m-%d', '%d/%m/%Y', 'Unix Timestamp', 'Saniyelik Index'
        ])
        layout.addWidget(self.time_format_combo, 1, 1)
        
        # Zaman birimi (eğer index ise)
        layout.addWidget(QLabel("Zaman Birimi:"), 2, 0)
        self.time_unit_combo = QComboBox()
        self.time_unit_combo.addItems(['saniye', 'milisaniye', 'mikrosaniye', 'nanosaniye'])
        layout.addWidget(self.time_unit_combo, 2, 1)
        
        return group
        
    def _create_button_panel(self):
        """Alt buton paneli oluştur."""
        layout = QHBoxLayout()
        
        # Önizleme yenile butonu
        self.refresh_btn = QPushButton("🔄 Önizlemeyi Yenile")
    ...
    def _setup_connections(self):
        ...
        self.start_row_spinbox.valueChanged.connect(self._on_settings_changed)
    ...
    def _update_metadata_display(self, metadata):
        """Metadata görüntüsünü güncelle."""
        info_text = f"""
📊 Veri Bilgileri:
• Boyut: {metadata['shape'][0]} satır × {metadata['shape'][1]} sütun
• Dosya Boyutu: {metadata['file_size'] / (1024*1024):.1f} MB
