...
class DataPreviewThread(QThread):
    ...
    def __init__(self, file_path: str, encoding: str = 'utf-8', delimiter: str = ',', 
        ...
        self.start_row = start_row  # Dosya başından atlanacak satır sayısı
    ...
    def run(self):
        ...
        try:
            ...
            if file_ext == '.csv':
                ...
                # Doğru mantık: skiprows ile baştan atla, sonra header'ı belirle
                skip_rows_list = None
                if self.start_row > 0:
                    skip_rows_list = list(range(self.start_row))
                ...
                df = pd.read_csv(
                    ...
                    skiprows=skip_rows_list,  # Baştan atlanacak satırlar
                    nrows=100,  # Sadece ilk 100 satır
                    low_memory=False
                )
            elif file_ext in ['.xlsx', '.xls']:
                # Excel dosyası
                # Aynı mantık: skiprows ile baştan atla, sonra header'ı belirle
                skip_rows_list = None
                if self.start_row > 0:
                    skip_rows_list = list(range(self.start_row))
...
class DataImportDialog(QDialog):
    ...
    def _create_row_settings(self):
        ...
        # Dosya başından atlanacak satır (metadata, boş satırlar vs.)
        layout.addWidget(QLabel("Dosya Başından Atla:"), 0, 0)
        self.skip_rows_spinbox = QSpinBox()
        self.skip_rows_spinbox.setRange(0, 100)
        self.skip_rows_spinbox.setValue(0)
        self.skip_rows_spinbox.setToolTip("Dosyanın başından kaç satır atlanacak (metadata, boş satırlar)")
        layout.addWidget(self.skip_rows_spinbox, 0, 1)
        
        # Veri başlangıç satırı (header'dan sonra)
        layout.addWidget(QLabel("Veri Başlangıç Satırı:"), 1, 0)
        ...
        layout.addWidget(self.start_row_spinbox, 1, 1)
