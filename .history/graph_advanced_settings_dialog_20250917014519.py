...
from PyQt5.QtWidgets import (
    ...
    QDoubleSpinBox, QCheckBox, QComboBox, QSlider, QTabWidget,
...
class ParameterFiltersPanel(QWidget):
    """Panel for configuring parameter filters (range, type, etc.)."""
    ...
    def _setup_ui(self):
        """Setup the parameter filters panel UI with tabs."""
        layout = QVBoxLayout(self)
        layout.setContentsMargins(20, 20, 20, 20)
        layout.setSpacing(15)
        
        # Title
        title = QLabel("🔍 Parameter Filters")
        title.setStyleSheet("""
            QLabel {
                font-size: 16px;
                font-weight: bold;
        ...
        # Description
        desc = QLabel("Apply advanced filters to refine which parameters are displayed based on various criteria.")
        desc.setStyleSheet("""
            QLabel {
        ...
        # Create individual filter tabs
        self._create_range_filter_tab()
    ...
    def _create_range_filter_tab(self):
        """Create the advanced value range filter tab with integrated parameter and range selection."""
        tab = QWidget()
        ...
        # Description
        desc = QLabel("Create range conditions for parameters. All conditions are combined with AND logic.")
        desc.setStyleSheet("""
            QLabel {
                color: rgba(230, 243, 255, 0.8);
                font-size: 12px;
        ...
        self.concatenated_mode_rb = QCheckBox("Concatenated Display (Join matching segments continuously)")
        ...
        self.tab_widget.addTab(tab, "📈 Range")
    ...
    def _create_type_filter_tab(self):
        """Create the signal type filter tab."""
        tab = QWidget()
        layout = QVBoxLayout(tab)
        layout.setContentsMargins(15, 15, 15, 15)
        layout.setSpacing(12)
        ...
        # Type controls
        type_group = QGroupBox("🏷️ Signal Type Settings")
        type_group.setStyleSheet(self._get_group_style())
        type_layout = QFormLayout(type_group)
        
        self.signal_type_combo = QComboBox()
        self.signal_type_combo.addItems(["All Types", "Analog", "Digital", "Calculated", "Derived"])
        
        type_layout.addRow("Signal Type:", self.signal_type_combo)
        
        layout.addWidget(type_group)
        layout.addStretch()
        
        self.tab_widget.addTab(tab, "🏷️ Type")
        
    def _create_frequency_filter_tab(self):
        """Create the frequency filter tab."""
        tab = QWidget()
        layout = QVBoxLayout(tab)
        layout.setContentsMargins(15, 15, 15, 15)
        layout.setSpacing(12)
        ...
        freq_group.setStyleSheet(self._get_group_style())
        ...
        self.tab_widget.addTab(tab, "⚡ Frequency")
        
    def _create_quality_filter_tab(self):
        """Create the quality filter tab."""
        tab = QWidget()
        layout = QVBoxLayout(tab)
        layout.setContentsMargins(15, 15, 15, 15)
        layout.setSpacing(12)
        ...
        quality_group.setStyleSheet(self._get_group_style())
        quality_layout = QVBoxLayout(quality_group)
        
        self.good_quality_cb = QCheckBox("Good Quality Signals")
        self.bad_quality_cb = QCheckBox("Bad Quality Signals")
        self.uncertain_quality_cb = QCheckBox("Uncertain Quality Signals")
        
        for cb in [self.good_quality_cb, self.bad_quality_cb, self.uncertain_quality_cb]:
            cb.setChecked(True)
...
class GraphAdvancedSettingsDialog(QDialog):
    ...
    def _create_sidebar(self, parent):
        ...
        self.filters_btn = ModernSidebarButton("Parameter Filters", "🔍")
    ...
    def _on_nav_button_clicked(self, button):
        ...
        panel_names = ["Parameters", "Parameter Filters", "Static Limits", "Deviation"]
