...
class GraphAdvancedSettingsDialog(QDialog):
    ...
    def __init__(self, graph_index: int, all_signals: List[str], visible_signals: List[str] = None, 
                 saved_filter_data: dict = None, parent=None):
        super().__init__(parent)
    ...
    def _update_summary_content(self):
        ...
        elif current_index == 3:  # Deviation
            self._show_deviation_summary()
    ...
    def _show_parameters_summary(self):
        ...
        if selected_signals:
            for signal in selected_signals:
                ...
                label.setStyleSheet("color: #e6f3ff; font-size: 11px; padding: 2px;")
                label.setWordWrap(True)
                layout.addWidget(label)
        else:
            label = QLabel("No parameters selected")
            label.setStyleSheet("color: #888; font-style: italic; font-size: 11px;")
            layout.addWidget(label)
        ...
        try:
            ...
            if conditions:
                for i, condition in enumerate(conditions):
                    ...
                    param_label.setStyleSheet("color: #4a90e2; font-weight: bold; font-size: 11px; margin-top: 5px;")
                    ...
                    for range_info in ranges:
                        ...
                        range_label.setStyleSheet("color: #e6f3ff; font-size: 10px; padding-left: 10px;")
                        layout.addWidget(range_label)
                        
                # Mode info
                mode = filter_conditions.get('mode', 'segmented')
                mode_label = QLabel(f"Mode: {mode.title()}")
                mode_label.setStyleSheet("color: #888; font-size: 10px; margin-top: 8px; font-style: italic;")
        ...
        try:
            ...
            if limits_config:
                for signal_name, limits in limits_config.items():
                    ...
                    signal_label.setStyleSheet("color: #4a90e2; font-weight: bold; font-size: 11px; margin-top: 5px;")
                    ...
                    if limits.get('warning_min') != 0 or limits.get('warning_max') != 0:
                        ...
                        warning_label.setStyleSheet("color: #ffa500; font-size: 10px; padding-left: 10px;")
                        layout.addWidget(warning_label)
                    
                    # Error limits
                    if limits.get('error_min') != 0 or limits.get('error_max') != 0:
                        error_text = f"  ❌ Error: {limits.get('error_min', 0):.2f} - {limits.get('error_max', 0):.2f}"
                        error_label = QLabel(error_text)
                        error_label.setStyleSheet("color: #ff6b6b; font-size: 10px; padding-left: 10px;")
    ...
    def _show_deviation_summary(self):
        ...
        try:
            ...
            if global_settings:
                ...
                method_label.setStyleSheet("color: #e6f3ff; font-size: 11px; font-weight: bold;")
                layout.addWidget(method_label)
                
                # Window size and sensitivity
                window_label = QLabel(f"Window: {global_settings.get('window_size', 0)} samples")
                window_label.setStyleSheet("color: #e6f3ff; font-size: 10px;")
                layout.addWidget(window_label)
                
                sensitivity_label = QLabel(f"Sensitivity: {global_settings.get('sensitivity', 0)}%")
                sensitivity_label.setStyleSheet("color: #e6f3ff; font-size: 10px;")
                ...
                if global_settings.get('outlier_detection', False):
                    ...
                    outlier_label.setStyleSheet("color: #ffa500; font-size: 10px;")
            ...
            if signal_settings:
                ...
                signals_label.setStyleSheet("color: #4a90e2; font-weight: bold; font-size: 11px; margin-top: 8px;")
                ...
                for signal_name, settings in list(signal_settings.items())[:3]:  # Show max 3 signals
                    ...
                    signal_label.setStyleSheet("color: #e6f3ff; font-size: 10px; padding-left: 10px;")
                    signal_label.setWordWrap(True)
                    layout.addWidget(signal_label)
                
                if len(signal_settings) > 3:
                    more_label = QLabel(f"... and {len(signal_settings) - 3} more")
                    more_label.setStyleSheet("color: #888; font-size: 10px; font-style: italic; padding-left: 10px;")
