...
class GraphAdvancedSettingsDialog(QDialog):
    ...
    def _show_advanced_deviation_summary(self):
        """Show advanced deviation summary."""
        group = QGroupBox("📈 Advanced Deviation Settings (Demo UI)")
        group.setStyleSheet(self._get_summary_group_style())
        layout = QVBoxLayout(group)
        
        try:
            deviation_settings = self.advanced_deviation_panel.get_deviation_settings()
    ...
    def _setup_connections(self):
        ...
        if hasattr(self.basic_deviation_panel, 'deviation_settings_changed'):
            ...
            # Also connect to apply signal for immediate application
            self.basic_deviation_panel.deviation_settings_changed.connect(
                lambda settings: self.basic_deviation_applied.emit(self.graph_index, settings)
            )
