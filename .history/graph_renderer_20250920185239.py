...
class GraphRenderer:
    ...
    def _draw_signal_limit_lines(self, plot_widget, graph_index: int, signal_name: str, limits: Dict[str, float], x_min: float, x_max: float):
        ...
        try:
            ...
            # Create dashed pen for limit lines - more visible
            limit_pen = pg.mkPen(color='#FFA500', width=3, style=pg.QtCore.Qt.DashLine)
    ...
    def _highlight_limit_violations(self, plot_widget, graph_index: int, signal_name: str, limits: Dict[str, float]):
        ...
        try:
            ...
            # Create violation highlight pen - use same color as signal but dashed and thicker
            signal_color = self._get_signal_color(signal_name)
            violation_pen = pg.mkPen(color=signal_color, width=4, style=pg.QtCore.Qt.DashLine)
