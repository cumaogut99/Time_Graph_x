...
class GraphRenderer:
    ...
    def _apply_basic_deviation_to_graph(self, tab_index: int, graph_index: int, deviation_settings: Dict[str, Any]):
        ...
        try:
            ...
            # Get plot widgets directly from the parent TimeGraphWidget
            plot_widgets = self.parent_widget.plot_widgets
            if graph_index >= len(plot_widgets):
        ...
        try:
            ...
            if (deviation_results['alerts'] and 
                ...
                if alert_x and alert_y:
                    ...
                    logger.info(f"Added {len(alert_x)} alert points for {signal_name}")
                ...
                for i, segment in enumerate(deviation_results['red_segments']):
                    ...
                    if start_idx < len(x_data) and end_idx < len(x_data):
                        ...
                        if len(segment_x) > 0 and len(segment_y) > 0:
                            ...
                            logger.info(f"Added red segment {i+1} for {signal_name} from index {start_idx} to {end_idx}")
