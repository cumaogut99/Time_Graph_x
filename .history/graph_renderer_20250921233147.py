...
class GraphRenderer:
    ...
    def apply_segmented_filter(self, container, graph_index: int, time_segments: List[Tuple[float, float]]):
        ...
        for signal_name in visible_signals:
            ...
            # Create optimized segmented data with NaN separators
            segmented_x = []
            segmented_y = []
            segments_found = 0
            ...
            for i, (segment_start, segment_end) in enumerate(time_segments):
                ...
                if len(segment_x) > 0:
                    ...
                    # Add NaN separator between segments (except for last segment)
            ...
            if segmented_x:
                ...
                plot_widget.plot(
                    np.array(segmented_x), 
                    np.array(segmented_y), 
                    pen=color, 
                    name=signal_name,
                    connect='finite'  # Only connect finite values, skip NaN
                )
                logger.info(f"[SEGMENTED DEBUG] Signal {signal_name}: plotted {segments_found} segments as single optimized line")
            else:
        ...
        try:
            ...
            for signal_name in signals_to_process:
                if signal_name in all_signals_data:
                    ...
                    logger.info(f"[DEVIATION DEBUG] Calculating deviation for {signal_name}")
                    deviation_results = self._calculate_basic_deviation(y_data, deviation_settings)
                    logger.info(f"[DEVIATION DEBUG] Results: {list(deviation_results.keys())}")

                    # Visualize results
                    logger.info(f"[DEVIATION DEBUG] Visualizing results for {signal_name}")
