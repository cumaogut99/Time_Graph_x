            ...
            if hasattr(plot_item, 'setPen'):
                try:
                    ...
                    if current_pen is not None:
                        ...
                        new_pen = pg.mkPen(color=current_pen.color(), width=width, style=current_pen.style())
                        plot_item.setPen(new_pen)
                        logger.debug(f"Updated line width to {width} for signal: {signal_key}")
                except Exception as e:
                    logger.debug(f"Could not update line width for signal {signal_key}: {e}")
        logger.info(f"Set line width to {width} for all signals")
