        ...
        for signal_key, plot_item in self.current_signals.items():
            if hasattr(plot_item, 'setPen'):
                try:
                    ...
                    logger.debug(f"Processing {signal_key}: {type(plot_item)}")
                    
                    # Try multiple methods to get the current pen
                    current_pen = None
                    
                    # Method 1: Try opts dictionary
                    if hasattr(plot_item, 'opts') and 'pen' in plot_item.opts:
                        current_pen = plot_item.opts['pen']
                        logger.debug(f"Got pen from opts for {signal_key}")
                    
                    # Method 2: Try pen() method
                    elif hasattr(plot_item, 'pen'):
                        try:
                            current_pen = plot_item.pen()
                            logger.debug(f"Got pen from pen() method for {signal_key}")
                        except:
                            pass
                    
                    # Method 3: Try curve.pen() if it exists
                    elif hasattr(plot_item, 'curve') and hasattr(plot_item.curve, 'pen'):
                        try:
                            current_pen = plot_item.curve.pen()
                            logger.debug(f"Got pen from curve.pen() for {signal_key}")
                        except:
                            pass
                    
                    if current_pen is not None:
                        # Create new pen with same color but different width
                        pen_color = current_pen.color()
                        pen_style = current_pen.style() if hasattr(current_pen, 'style') else None
                        ...
                        updated_count += 1
                        logger.debug(f"Updated line width to {width} for signal: {signal_key}")
                    else:
                        logger.debug(f"Could not get pen for signal: {signal_key}")
                        
                except Exception as e:
                    logger.debug(f"Exception updating line width for signal {signal_key}: {e}")
