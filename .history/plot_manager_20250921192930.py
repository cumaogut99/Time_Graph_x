        ...
        for signal_key, plot_item in self.current_signals.items():
            try:
                ...
                if hasattr(plot_item, 'opts') and 'pen' in plot_item.opts:
                    ...
                    updated_count += 1
                    logger.info(f"Updated line width to {width} for signal: {signal_key}")
                
                # Method 2: Doğrudan setPen ile yeni pen oluştur (renk kaybı riski var)
                elif hasattr(plot_item, 'setPen'):
                    # Mevcut rengi korumaya çalış
                    try:
                        # Eğer curve varsa ondan rengi al
                        ...
                        plot_item.setPen(new_pen)
                        updated_count += 1
                        logger.info(f"Updated line width to {width} for signal: {signal_key} (fallback method)")
                    except:
                        # Son çare: sadece width ile pen oluştur
                        new_pen = pg.mkPen(width=width)
                        plot_item.setPen(new_pen)
                        updated_count += 1
                        logger.info(f"Updated line width to {width} for signal: {signal_key} (basic method)")
