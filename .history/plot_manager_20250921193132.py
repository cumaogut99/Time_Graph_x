...
class PlotManager(QObject):
    ...
    def set_line_width(self, width: int):
        ...
        # Renk paletini yeniden oluştur (her sinyal için farklı renk)
        signal_colors = {}
        color_index = 0
        
        for signal_key, plot_item in self.current_signals.items():
            try:
                # Her sinyal için benzersiz renk ata
                if signal_key not in signal_colors:
                    signal_colors[signal_key] = self._get_next_color(color_index)
                    color_index += 1
                
                pen_color = signal_colors[signal_key]
                
                # PyQtGraph PlotDataItem için doğru yöntem:
                # Önce mevcut pen'i al, sonra yeni pen oluştur
                
                # Method 1: opts dictionary'den pen al ve rengi koru
                if hasattr(plot_item, 'opts') and 'pen' in plot_item.opts:
                    current_pen = plot_item.opts['pen']
                    
                    # Mevcut rengi korumaya çalış
                    if hasattr(current_pen, 'color'):
                        try:
                            # Mevcut rengi al
                            existing_color = current_pen.color()
                            # QColor'dan hex string'e çevir
                            ...
                            # Fallback: önceden atanan rengi kullan
                            pass
                    
                    # Yeni pen oluştur - renk ve stil korunarak
                    if hasattr(current_pen, 'style'):
                        new_pen = pg.mkPen(color=pen_color, width=width, style=current_pen.style())
                    else:
                    ...
                    updated_count += 1
                    logger.info(f"Updated line width to {width} for signal: {signal_key} with color: {pen_color}")
                
                # Method 2: Doğrudan setPen ile yeni pen oluştur
                elif hasattr(plot_item, 'setPen'):
                    # Benzersiz renk ile pen oluştur
                    new_pen = pg.mkPen(color=pen_color, width=width)
                    plot_item.setPen(new_pen)
                    updated_count += 1
                    logger.info(f"Updated line width to {width} for signal: {signal_key} with fallback color: {pen_color}")
