...
class PlotManager(QObject):
            ...
            if enabled:
                ...
                if plot_widget not in self.tooltip_items:
                    tooltip_item = pg.TextItem(
                        text="",
                        color=(255, 255, 255),
                        fill=pg.mkBrush(0, 0, 0, 120),  # Semi-transparent black background
                ...
                # Connect mouse leave events to hide tooltip
                plot_widget.leaveEvent = lambda event, pw=plot_widget: self._on_mouse_left(pw)
                ...
                # Disconnect mouse events (this is tricky with PyQt signals)
                try:
    ...
    def _on_mouse_moved(self, pos, plot_widget):
        ...
        try:
            ...
            # Find signal values at this X position
            tooltip_text = self._generate_tooltip_text(x_pos, y_pos)
