...
class PlotManager(QObject):
    ...
    def _setup_context_menu(self, plot_widget):
        ...
        try:
            ...
            for i, action in enumerate(actions):
                if action.text() == "Mouse Mode" or "Mouse" in action.text():
                    insert_index = i
                    logger.info(f"Found Mouse Mode at index {i}, inserting before it")
                    break
                elif "Y Axis" in action.text():
                    # Insert after Y Axis (and any separator that follows)
                    insert_index = i + 1
                    # Skip separator if it exists
                    if insert_index < len(actions) and actions[insert_index].isSeparator():
                        insert_index += 1
                    logger.info(f"Found Y Axis at index {i}, inserting at index {insert_index}")
