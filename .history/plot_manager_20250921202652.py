...
class PlotManager(QObject):
    ...
    def _setup_context_menu(self, plot_widget):
        ...
        try:
            ...
            # Log existing actions for debug
            actions = original_menu.actions()
            logger.info(f"Original menu has {len(actions)} actions:")
            for i, action in enumerate(actions):
                logger.info(f"  {i}: '{action.text()}' (separator: {action.isSeparator()})")
                
            # Create our zoom action
            ...
            # Find the position to insert our action (after Y Axis, before Mouse Mode)
            insert_index = len(actions)  # Default to end if not found
            y_axis_found = False
            
            # Look for Y Axis menu or Mouse Mode to determine insertion point
            for i, action in enumerate(actions):
                action_text = action.text()
                logger.info(f"Checking action {i}: '{action_text}'")
                
                if action_text == "Mouse Mode" or "Mouse" in action_text:
                    insert_index = i
                    logger.info(f"Found Mouse Mode at index {i}, inserting before it")
                    break
                elif "Y axis" in action_text or action_text == "Y axis":
                    # Insert after Y Axis (and any separator that follows)
                    insert_index = i + 1
                    y_axis_found = True
                    logger.info(f"Found Y axis at index {i}, will insert at index {insert_index}")
                    # Continue looking for Mouse Mode to insert before it instead
                    continue
            
            # If we found Y axis but not Mouse Mode, insert right after Y axis
            if y_axis_found and insert_index > len(actions) - 1:
                # Find Y axis again and insert right after it
                for i, action in enumerate(actions):
                    if "Y axis" in action.text() or action.text() == "Y axis":
                        insert_index = i + 1
                        break
            
            logger.info(f"Final insert index: {insert_index}")
            
            # Insert our action at the determined position
            if insert_index < len(actions):
                original_menu.insertAction(actions[insert_index], zoom_action)
                logger.info(f"Inserted Zoom to Cursors before action: '{actions[insert_index].text()}'")
            else:
                original_menu.addAction(zoom_action)
                logger.info("Added Zoom to Cursors at the end")
                
            # Add separator after our action if the next action isn't already a separator
            if insert_index < len(actions) and not actions[insert_index].isSeparator():
                separator = original_menu.insertSeparator(actions[insert_index])
                logger.info("Added separator after Zoom to Cursors")
