...
class PlotManager(QObject):
    ...
    def _setup_context_menu(self, plot_widget):
        ...
        try:
            # Get the original context menu to copy its actions
            original_menu = plot_widget.plotItem.ctrlMenu
            
            logger.info(f"Setting up context menu for plot widget. Original menu exists: {original_menu is not None}")
            
            if not original_menu:
                logger.warning("No original context menu found")
                return
            
            # Create a completely new context menu
            new_menu = QMenu()
            
            # Get original actions
            original_actions = original_menu.actions()
            logger.info(f"Original menu has {len(original_actions)} actions:")
            for i, action in enumerate(original_actions):
                logger.info(f"  {i}: '{action.text()}' (separator: {action.isSeparator()})")
            
            # Add original actions one by one, inserting our action at the right place
            zoom_action_added = False
            
            for i, original_action in enumerate(original_actions):
                # Add the original action first
                if original_action.isSeparator():
                    new_menu.addSeparator()
                else:
                    # Clone the action
                    new_action = QAction(original_action.text(), self)
                    new_action.setCheckable(original_action.isCheckable())
                    new_action.setChecked(original_action.isChecked())
                    new_action.setEnabled(original_action.isEnabled())
                    new_action.setToolTip(original_action.toolTip())
                    
                    # Handle submenus
                    if original_action.menu():
                        submenu = QMenu(original_action.text())
                        # Copy submenu actions
                        for sub_action in original_action.menu().actions():
                            if sub_action.isSeparator():
                                submenu.addSeparator()
                            else:
                                cloned_sub = QAction(sub_action.text(), self)
                                cloned_sub.setCheckable(sub_action.isCheckable())
                                cloned_sub.setChecked(sub_action.isChecked())
                                cloned_sub.setEnabled(sub_action.isEnabled())
                                cloned_sub.triggered.connect(sub_action.trigger)
                                submenu.addAction(cloned_sub)
                        new_action.setMenu(submenu)
                    else:
                        # Connect to original action's trigger
                        new_action.triggered.connect(original_action.trigger)
                    
                    new_menu.addAction(new_action)
                
                # After Y axis, add our zoom action
                if not zoom_action_added and original_action.text() == "Y axis":
                    ...
                    new_menu.addAction(zoom_action)
                    zoom_action_added = True
                    logger.info("Added Zoom to Cursors after Y axis")
                    
                    # Store reference for state updates
                    plot_widget._cursor_zoom_action = zoom_action
            
            # If we didn't add it yet (no Y axis found), add at the end
            if not zoom_action_added:
                zoom_action = QAction("🎯 Zoom to Cursors", self)
                zoom_action.setToolTip("Zoom to the range between dual cursors")
                zoom_action.triggered.connect(self._on_zoom_to_cursors)
                new_menu.addAction(zoom_action)
                plot_widget._cursor_zoom_action = zoom_action
                logger.info("Added Zoom to Cursors at the end (Y axis not found)")
            
            # Replace the original context menu
            plot_widget.plotItem.ctrlMenu = new_menu
            
            # Connect to menu about to show to update action state
            new_menu.aboutToShow.connect(lambda: self._update_context_menu_state(plot_widget))
            
            logger.info("Context menu setup completed successfully")
    ...
    def _on_zoom_to_cursors(self):
        ...
        try:
            ...
            if success:
                logger.info("Successfully zoomed to cursor range from context menu")
            else:
