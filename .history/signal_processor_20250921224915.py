        ...
        with QMutexLocker(self.mutex):
            for signal_name, data in filtered_data.items():
                if signal_name in self.signal_data:
                    ...
                    self.signal_data[signal_name]['x_data'] = data['time']
                    self.signal_data[signal_name]['y_data'] = data['values']
                    
                    # Clear related caches since data changed
                    self._clear_cache(signal_name)
                    
                    logger.debug(f"Updated filtered data for signal '{signal_name}' with {len(data['values'])} points")
                else:
