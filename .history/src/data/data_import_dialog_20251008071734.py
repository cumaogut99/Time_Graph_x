...
class DataPreviewThread(QThread):
    ...
    def __init__(self, file_path: str, encoding: str = 'latin-1', delimiter: str = ',', 
        ...
        # If header_row is set and different from start_row, we need to skip rows
        if header_row >= 0:
            # Has header: skip rows between header and start_row
            self.skip_rows = max(0, start_row - header_row - 1)
            self.has_header = True
        else:
            # No header: skip rows until start_row
            self.skip_rows = start_row
...
class DataImportDialog(QDialog):
    ...
    def _refresh_preview(self):
        ...
        self.preview_thread = DataPreviewThread(
            self.file_path, encoding, delimiter, header_row
        )
