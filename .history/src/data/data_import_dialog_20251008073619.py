...
class DataPreviewThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            if file_ext == '.csv':
                # Check if file needs line-quote preprocessing
                if self._needs_line_quote_stripping():
                    logger.info("Preview: Line-wrapped quotes detected, preprocessing...")
                    df = self._read_csv_with_quote_stripping()
                else:
                    df = pl.read_csv(
                        self.file_path,
                        ...
                        n_rows=100,  # Sadece ilk 100 satır
                        low_memory=False,
                        quote_char='"',  # Handle quoted fields properly
                        infer_schema_length=100,  # Limit schema inference to preview rows
                        ignore_errors=True,  # Continue on parsing errors
                        try_parse_dates=False  # Don't auto-parse dates in preview
                    )
    ...
    def _read_csv_with_quote_stripping(self):
        ...
        try:
            ...
            with open(self.file_path, 'r', encoding=self.encoding, errors='replace') as f:
                ...
                for line in f:
                    ...
                    # Limit to enough rows for header + skip + 100 data rows
                    max_rows = self.start_row + 100 + (1 if self.has_header else 0)
            ...
            df = pl.read_csv(
                ...
                skip_rows=self.skip_rows,
                low_memory=False,
                infer_schema_length=100,  # Limit schema inference
                ignore_errors=True,  # Continue on parsing errors
                try_parse_dates=False  # Don't auto-parse dates in preview
            )
            
            logger.info(f"Preview with quote stripping: {df.shape}")
            return df
            
        except Exception as e:
            logger.error(f"Preview quote stripping failed: {e}")
