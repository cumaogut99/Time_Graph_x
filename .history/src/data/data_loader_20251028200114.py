...
class DataLoader(QObject):
        ...
        if file_ext == '.csv':
            try:
                ...
                if df is None:
                    ...
                    if self._needs_line_quote_stripping(file_path, csv_opts['encoding']):
                        logger.info("File has line-wrapped quotes, preprocessing...")
                        df = self._read_csv_with_quote_stripping(file_path, csv_opts)
                    else:
    ...
    def _needs_line_quote_stripping(self, file_path, encoding):
        """
    ...
    def _read_csv_with_quote_stripping(self, file_path, csv_opts):
        """
        ...
        try:
            ...
            # Create modified opts without quote_char (already processed)
            clean_opts = csv_opts.copy()
            clean_opts.pop('quote_char', None)  # Remove quote handling
            
            df = pl.read_csv(io.StringIO(cleaned_content), **clean_opts)
            logger.info(f"Successfully read CSV with quote stripping: {df.shape}")
            return df
            
        except Exception as e:
            logger.error(f"Quote stripping failed, trying normal read: {e}")
            return pl.read_csv(file_path, **csv_opts)
            ...
            if time_column_name and time_column_name in df.columns:
                ...
                if time_format == 'Unix Timestamp':
                    ...
                    if original_series.dtype in [pl.Float32, pl.Float64, pl.Int32, pl.Int64]:
                        ...
                        logger.info(f"Time column '{time_column_name}' is already Datetime (parsed by Polars)")
                        converted_series = original_series
                    else:
                        ...
                        datetime_formats = [
                            None,  # Auto-detect (Polars default)
                        ...
                        for fmt in datetime_formats:
                            try:
                                ...
                                if converted_series is not None and converted_series.null_count() < len(converted_series):
                                    logger.info(f"Datetime parsing successful with format: {fmt or 'auto'}")
                                    break
                            except Exception as e:
                                continue
                        
                        # If all formats failed, try numeric
                        if converted_series is None or converted_series.null_count() == len(converted_series):
                            logger.warning(f"'{time_column_name}' datetime conversion failed, trying numeric")
