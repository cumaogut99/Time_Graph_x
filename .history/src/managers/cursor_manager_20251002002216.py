...
class CursorManager(QObject):
    ...
    def __init__(self, plot_widgets):
        ...
        self.cursor1_viewport_ratio = 1/3  # Default at 1/3 of visible range
        self.cursor2_viewport_ratio = 2/3  # Default at 2/3 of visible range
        self.lock_cursors_to_viewport = False  # TEMPORARILY DISABLED FOR TESTING
        self._updating_cursor_positions = False  # Flag to prevent recursive updates
        self._view_range_signals_blocked = False # Flag to temporarily block signals
        ...
        for plot_widget in self.plot_widgets:
            ...
            # Connect to view range change signal to keep cursors in viewport
    ...
    def _sync_dual_cursors_1(self, moved_cursor):
        ...
        # Update viewport ratio if viewport lock is enabled
        if self.lock_cursors_to_viewport and self.plot_widgets:
            try:
                plot_widget = self.plot_widgets[0]
                view_range = plot_widget.getViewBox().viewRange()[0]
                view_min, view_max = view_range
                view_width = view_max - view_min
                
                if view_width > 0:
                    self.cursor1_viewport_ratio = (snapped_pos - view_min) / view_width
                    self.cursor1_viewport_ratio = max(0.0, min(1.0, self.cursor1_viewport_ratio))
            except Exception as e:
                logger.debug(f"Failed to update cursor1 viewport ratio: {e}")
    ...
    def _sync_dual_cursors_2(self, moved_cursor):
        ...
        if self.lock_cursors_to_viewport and self.plot_widgets:
            try:
                ...
                if view_width > 0:
                    self.cursor2_viewport_ratio = (snapped_pos - view_min) / view_width
                    self.cursor2_viewport_ratio = max(0.0, min(1.0, self.cursor2_viewport_ratio))
            except Exception as e:
                logger.debug(f"Failed to update cursor2 viewport ratio: {e}")
    ...
    def _on_view_range_changed(self, view_box, range_data):
        """
        Handle view range changes (pan/zoom) to keep cursors locked to viewport.
        Performance-optimized to prevent recursive updates.
        """
        # Skip if signals are temporarily blocked (e.g., during auto-range)
        if self._view_range_signals_blocked:
            return
            
        # Skip if not in dual mode or cursors don't exist
        if self.current_mode != "dual":
            return
            
        # Skip if viewport locking is disabled
        if not self.lock_cursors_to_viewport:
            return
            
        # Skip if we're currently updating cursors (prevent recursion)
        if self._updating_cursor_positions:
            return
            
        # Check if cursors exist
        if not self.dual_cursors_1 or not self.dual_cursors_2:
            return
            
        try:
            self._updating_cursor_positions = True
            
            # Get current view range
            x_range = range_data[0]
            view_min, view_max = x_range
            view_width = view_max - view_min
            ...
            # Get current cursor positions
            cursor1_pos = self.dual_cursors_1[0].value()
            cursor2_pos = self.dual_cursors_2[0].value()
            
            # CRITICAL FIX: Check if cursors are already within the new view range
            # If they are, just update the viewport ratios without moving cursors
            # This prevents X-axis zoom issues when autoRange() is called
            cursor1_in_range = view_min <= cursor1_pos <= view_max
            cursor2_in_range = view_min <= cursor2_pos <= view_max
            
            if cursor1_in_range and cursor2_in_range:
                # Cursors are already in view - just update ratios, don't move cursors
                self.cursor1_viewport_ratio = (cursor1_pos - view_min) / view_width
                self.cursor2_viewport_ratio = (cursor2_pos - view_min) / view_width
                logger.debug(f"Cursors in view - updated ratios: C1={self.cursor1_viewport_ratio:.3f}, C2={self.cursor2_viewport_ratio:.3f}")
                return
            
            # Calculate new cursor positions based on viewport ratios
            new_pos1 = view_min + (view_width * self.cursor1_viewport_ratio)
            new_pos2 = view_min + (view_width * self.cursor2_viewport_ratio)
            
            # Update cursor positions without triggering signals
            for cursor in self.dual_cursors_1:
                cursor.blockSignals(True)
                cursor.setPos(new_pos1)
                cursor.blockSignals(False)
                
            for cursor in self.dual_cursors_2:
                cursor.blockSignals(True)
                cursor.setPos(new_pos2)
                cursor.blockSignals(False)
            
            # Emit cursor positions once after update
            self._emit_cursor_positions()
            
        except Exception as e:
            logger.warning(f"Error updating cursor positions on view change: {e}")
        finally:
            self._updating_cursor_positions = False
    
    def block_view_range_signals(self, block: bool):
        """Temporarily block or unblock the _on_view_range_changed slot."""
        self._view_range_signals_blocked = block
        logger.debug(f"View range signals {'blocked' if block else 'unblocked'}")

    def reset_cursors_to_viewport(self):
        """
        Manually repositions the cursors to their default viewport ratios (1/3 and 2/3).
        This should be called after a full autoRange operation.
        """
        if self.current_mode != "dual" or not self.plot_widgets or not self.dual_cursors_1 or not self.dual_cursors_2:
            return
        ...
        try:
            ...
            # DEBUG: Log current view range
            logger.debug(f"[DEBUG] reset_cursors_to_viewport - view_range: [{view_min}, {view_max}], width: {view_width}")
            
            if view_width <= 0:
                logger.warning(f"[DEBUG] Invalid view_width: {view_width}, aborting cursor reset")
                return
                
            # Reset to default ratios
            self.cursor1_viewport_ratio = 1/3
            self.cursor2_viewport_ratio = 2/3
            
            # Calculate new positions
            new_pos1 = view_min + (view_width * self.cursor1_viewport_ratio)
            new_pos2 = view_min + (view_width * self.cursor2_viewport_ratio)
            
            logger.debug(f"[DEBUG] New cursor positions: C1={new_pos1:.3f}, C2={new_pos2:.3f}")
            ...
            self._emit_cursor_positions()
            logger.info(f"Cursors reset to default viewport positions after auto-range: C1={new_pos1:.3f}, C2={new_pos2:.3f}")

        except Exception as e:
            logger.error(f"Failed to reset cursors to viewport: {e}")
        finally:
            self._updating_cursor_positions = False
    
    def set_viewport_lock(self, enabled: bool):
        """
        Enable or disable viewport locking for cursors.
        
        Args:
            enabled: If True, cursors stay at fixed viewport positions during pan/zoom.
                    If False, cursors stay at fixed data coordinates.
        """
        self.lock_cursors_to_viewport = enabled
        
        # If enabling, update viewport ratios based on current cursor positions
        if enabled and self.dual_cursors_1 and self.dual_cursors_2:
            try:
                # Get first plot widget for reference
                ...
                if plot_widget:
                    ...
                    if view_width > 0:
                        # Calculate current viewport ratios
                        pos1 = self.dual_cursors_1[0].value()
                        pos2 = self.dual_cursors_2[0].value()
                        
                        self.cursor1_viewport_ratio = (pos1 - view_min) / view_width
                        self.cursor2_viewport_ratio = (pos2 - view_min) / view_width
                        
                        # Clamp ratios to valid range
                        self.cursor1_viewport_ratio = max(0.0, min(1.0, self.cursor1_viewport_ratio))
                        self.cursor2_viewport_ratio = max(0.0, min(1.0, self.cursor2_viewport_ratio))
                        
                        logger.info(f"Viewport lock enabled. Cursor ratios: {self.cursor1_viewport_ratio:.3f}, {self.cursor2_viewport_ratio:.3f}")
            except Exception as e:
                logger.warning(f"Failed to calculate viewport ratios: {e}")
        
        logger.info(f"Cursor viewport lock {'enabled' if enabled else 'disabled'}")