...
class PlotManager(QObject):
    ...
    def _rebuild_ui(self):
        ...
        for i in range(self.subplot_count):
            ...
            # Apply global autoscale setting
    ...
    def _find_closest_signal_to_cursor(self, x_pos: float, y_pos: float, plot_widget):
        ...
        try:
            ...
            if not hasattr(self.parent, 'signal_processor'):
                logger.debug("No signal_processor on parent")
                return None
            ...
            if not signal_names:
                logger.debug("No signal names found")
                return None
            
            logger.debug(f"Finding closest signal - Available signals: {signal_names}, current_signals keys: {list(self.current_signals.keys())}, plot_index: {plot_index}")
