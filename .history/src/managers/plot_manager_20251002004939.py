...
class PlotManager(QObject):
    ...
    def reset_view(self):
        ...
        # CRITICAL FIX: X-axis is linked across all plots, calculate min/max across ALL plots
        ...
        if x_min != float('inf') and x_max != float('-inf'):
            for plot_widget in self.plot_widgets:
                plot_widget.setXRange(x_min, x_max, padding=0.02)
            logger.info(f"[RESET VIEW] X-axis set to [{x_min:.2f}, {x_max:.2f}] for all {len(self.plot_widgets)} linked plots")
        else:
            logger.warning("[RESET VIEW] No valid X data found in any plot")
