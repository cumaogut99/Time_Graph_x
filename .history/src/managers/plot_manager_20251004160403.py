...
class PlotManager(QObject):
    ...
    def add_signal(self, name: str, x_data: np.ndarray, y_data: np.ndarray, 
        ...
        # Store original data range BEFORE downsampling
        ...
        # CRITICAL FIX: If data was downsampled, ensure view includes ALL original data range
        # This fixes the issue where initial view doesn't show all data until zoom out
        if downsampled:
            try:
                ...
                # Expand view to include original data range if needed
                # Add 5% padding for better visibility
                x_padding = (original_x_max - original_x_min) * 0.05
                y_padding = (original_y_max - original_y_min) * 0.05
                
                new_x_range = [original_x_min - x_padding, original_x_max + x_padding]
                new_y_range = [original_y_min - y_padding, original_y_max + y_padding]
                
                # Only update if we're expanding (not shrinking) the view
                if current_range[0][0] > new_x_range[0] or current_range[0][1] < new_x_range[1]:
                    view_box.setXRange(new_x_range[0], new_x_range[1], padding=0)
                if current_range[1][0] > new_y_range[0] or current_range[1][1] < new_y_range[1]:
                    view_box.setYRange(new_y_range[0], new_y_range[1], padding=0)
                    
                logger.debug(f"View range adjusted for downsampled signal '{name}': X=[{original_x_min:.2f}, {original_x_max:.2f}]")
            except Exception as e:
                logger.warning(f"Could not adjust view range after downsampling: {e}")
    ...
    def reset_view(self):
        ...
        logger.info(f"Reset view - all {len(self.plot_widgets)} plots auto-ranged")
