...
class PlotManager(QObject):
    ...
    def _get_signal_data_for_restore(self) -> Dict[str, Dict]:
        ...
        signal_data = {}
        logger.info(f"📦 Storing signal data from {len(self.current_signals)} current signals")
            ...
            try:
                ...
                if hasattr(plot_item, 'xData') and plot_item.xData is not None and \
                   hasattr(plot_item, 'yData') and plot_item.yData is not None:
                    ...
                    logger.debug(f"  ✅ Stored signal '{original_name}' with {len(plot_item.xData)} data points")
            except Exception as e:
                logger.warning(f"Could not store data for signal '{name}': {e}")
        logger.info(f"📦 Total stored: {len(signal_data)} signals")
        return signal_data
