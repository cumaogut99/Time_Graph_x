        ...
        for name in signal_names:
            ...
            if name in signal_mapping:
                ...
                if plot_index < self.subplot_count:
                    ...
                    skipped_count += 1
                    logger.debug(f"⏸️ Signal '{name}' mapped to plot {plot_index} but only {self.subplot_count} plots exist. Keeping in mapping for future restore.")
            else:
                # No mapping found - don't auto-distribute to new graphs
                # New graphs should start empty until user explicitly assigns signals
                logger.debug(f"Signal '{name}' has no mapping - not restoring")
        
        logger.info(f"✅ Successfully restored {restored_count}/{len(signal_names)} signals (skipped {skipped_count} for future graphs)")
