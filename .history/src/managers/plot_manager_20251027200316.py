...
class PlotManager(QObject):
    ...
    def _get_signal_data_for_restore(self) -> Dict[str, Dict]:
        ...
        signal_data = {}
        logger.info(f"[RESTORE] Storing signal data from {len(self.current_signals)} current signals")
            ...
            try:
                ...
                if hasattr(plot_item, 'xData') and plot_item.xData is not None and \
                   hasattr(plot_item, 'yData') and plot_item.yData is not None:
                    ...
                    logger.debug(f"  [RESTORE] Stored signal '{original_name}' with {len(plot_item.xData)} data points")
            except Exception as e:
                logger.warning(f"Could not store data for signal '{name}': {e}")
        logger.info(f"[RESTORE] Total stored: {len(signal_data)} signals - Keys: {list(signal_data.keys())}")
        return signal_data
    ...
    def _restore_signals(self, signal_data: Dict[str, Dict]):
        ...
        if hasattr(self.parent, 'main_widget'):
            ...
            logger.info(f"[RESTORE] Accessing TimeGraphWidget via parent.main_widget")
        else:
            logger.error("[RESTORE] CRITICAL: Parent (GraphContainer) does not have main_widget!")
            return
        
        if hasattr(main_widget, 'graph_signal_mapping'):
            # Get current tab index
            current_tab = 0
            if hasattr(main_widget, 'get_active_tab_index'):
                current_tab = main_widget.get_active_tab_index()
            else:
                logger.warning("[RESTORE] TimeGraphWidget does not have get_active_tab_index method!")
            
            # Get signal mapping for current tab
            tab_mapping = main_widget.graph_signal_mapping.get(current_tab, {})
            logger.info(f"[RESTORE] Tab mapping for tab {current_tab}: {tab_mapping}")
            
            # Build reverse mapping: signal_name -> plot_index
            for plot_index, signals in tab_mapping.items():
                logger.debug(f"[RESTORE] Processing plot_index={plot_index}, signals={signals}")
                for signal_name in signals:
                    signal_mapping[signal_name] = plot_index
                    logger.debug(f"[RESTORE] Added to reverse mapping: '{signal_name}' -> {plot_index}")
            logger.info(f"[RESTORE] Reverse signal mapping: {signal_mapping}")
        else:
            logger.error("[RESTORE] CRITICAL: TimeGraphWidget does not have graph_signal_mapping attribute!")
        ...
        for name in signal_names:
            ...
            logger.debug(f"[RESTORE] Processing signal '{name}' from signal_data")
            
            # Only restore signals that have explicit mapping
            if name in signal_mapping:
                # Use saved mapping
                plot_index = signal_mapping[name]
                logger.debug(f"[RESTORE] Signal '{name}' mapped to plot {plot_index}")
                ...
                if plot_index < self.subplot_count:
                    ...
                    try:
                        ...
                        restored_count += 1
                        logger.info(f"[RESTORE] SUCCESS: Restored signal '{name}' to plot {plot_index}")
                    except Exception as e:
                        logger.error(f"[RESTORE] FAILED: Could not restore signal '{name}' to plot {plot_index}: {e}")
                else:
                    # Grafik henüz yok, sinyali atlayıp mapping'de sakla
                    skipped_count += 1
                    logger.info(f"[RESTORE] SKIPPED: Signal '{name}' mapped to plot {plot_index} but only {self.subplot_count} plots exist")
            else:
                # No mapping found - don't auto-distribute to new graphs
                # New graphs should start empty until user explicitly assigns signals
                logger.info(f"[RESTORE] NO MAPPING: Signal '{name}' has no mapping - not restoring")
        
        logger.info(f"[RESTORE] SUMMARY: Successfully restored {restored_count}/{len(signal_names)} signals (skipped {skipped_count} for future graphs)")
        
        # ✅ FIX: Grafik sayısı değiştiğinde otomatik view all yap
        if restored_count > 0:
            self.reset_view()
            logger.info(f"[RESTORE] Auto view all triggered after restoring {restored_count} signals")
    ...
    def set_tooltips_enabled(self, enabled: bool):
        ...
        self.tooltips_enabled = enabled
        logger.debug(f"PlotManager: Tooltips {'enabled' if enabled else 'disabled'}")
