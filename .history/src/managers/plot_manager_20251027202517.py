...
class PlotManager(QObject):
    ...
    def set_subplot_count(self, count: int):
        ...
        try:
            ...
            # DEBUG: Temporary log to diagnose the issue
            print(f"\n=== DEBUG: Graph Count Change ===")
            print(f"Old count: {self.subplot_count} -> New count: {count}")
            print(f"Signals to restore: {list(signal_data_to_restore.keys())}")
            print(f"Current signals before rebuild: {list(self.current_signals.keys())}")
    ...
    def _restore_signals(self, signal_data: Dict[str, Dict]):
        ...
        if hasattr(main_widget, 'graph_signal_mapping'):
            ...
            # DEBUG: Temporary log
            print(f"\n=== DEBUG: Restore Signals ===")
            print(f"Current tab index: {current_tab}")
            print(f"Tab mapping: {tab_mapping}")
            print(f"All graph_signal_mapping: {main_widget.graph_signal_mapping}")
            ...
            print(f"Reverse signal mapping: {signal_mapping}")
        else:
            ...
            if name in signal_mapping:
                ...
                if plot_index < self.subplot_count:
                    ...
                    try:
                        ...
                        restored_count += 1
                        print(f"✅ Restored: '{name}' to plot {plot_index}")
                    except Exception as e:
                        logger.error(f"Failed to restore signal '{name}' to plot {plot_index}: {e}")
                        print(f"❌ Failed: '{name}' to plot {plot_index}: {e}")
                else:
                    # Grafik henüz yok, sinyali atlayıp mapping'de sakla
                    skipped_count += 1
                    print(f"⏸️ Skipped: '{name}' (plot {plot_index} doesn't exist yet)")
            else:
                print(f"⚠️ NO MAPPING: '{name}' - not in signal_mapping")
        
        print(f"=== Restore Summary: {restored_count} restored, {skipped_count} skipped ===\n")
