...
class WidgetContainerManager(QObject):
    ...
    def _create_placeholder_widget(self):
        ...
        # Merkeze hizalanmış mesaj
        label = QLabel("Başlamak için bir dosya yükleyin\n\nDosya → Aç menüsünden CSV veya Excel dosyası seçin")
        ...
        # Placeholder'ı stacke ekle
        self.stacked_widget.addWidget(placeholder)
        self.placeholder_widget = placeholder
        
        logger.debug("Placeholder widget created")
    ...
    def switch_to_file_widget(self, file_index: int):
        ...
        # İlk widget oluşturulduğunda placeholder'ı gizle
        if hasattr(self, 'placeholder_widget') and self.placeholder_widget:
            if self.stacked_widget.indexOf(self.placeholder_widget) >= 0:
                self.stacked_widget.removeWidget(self.placeholder_widget)
                self.placeholder_widget.deleteLater()
                self.placeholder_widget = None
                logger.debug("Placeholder widget removed")
