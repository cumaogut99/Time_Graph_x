...
class GraphAdvancedSettingsDialog(QDialog):
    ...
    def __init__(self, graph_index: int, all_signals: List[str], visible_signals: List[str] = None, 
        ...
        # Load saved basic deviation data if available
    ...
    def _create_main_content(self, parent):
        """Create the main content area with stacked panels. OPTIMIZED: Lazy loading."""
        ...
        # PERFORMANCE: Lazy loading - panels oluşturulur ama içerik henüz yüklenmez
        # Sadece ilk panel (Parameters) hemen oluşturulur
        self.parameters_panel = ParametersPanel(self.all_signals, self.visible_signals, self)
        
        # Diğer paneller None olarak başlar, gerektiğinde oluşturulur
        ...
        # Sadece ilk paneli ekle
        self.stacked_widget.addWidget(self.parameters_panel)
        
        # Placeholder'lar için boş widget'lar ekle (index'leri korumak için)
        for _ in range(5):  # 5 adet daha panel var
            placeholder = QWidget()
    ...
    def _setup_connections(self):
        ...
        # Connect panel change signals to update summary (sadece parameters panel başlangıçta var)
        self.parameters_panel.signals_changed.connect(self._on_parameters_changed)
        
        # PERFORMANCE: Diğer panel'ların signal'ları lazy loading ile bağlanacak
    ...
    def _on_nav_button_clicked(self, button):
        """Handle navigation button clicks. OPTIMIZED: Lazy loading."""
        button_id = self.nav_buttons.id(button)
        
        # PERFORMANCE: Lazy load panel on demand
        self._ensure_panel_loaded(button_id)
    ...
    def _ensure_panel_loaded(self, panel_index: int):
        """PERFORMANCE: Lazy load panel on first access."""
        # Panel 0 (Parameters) already loaded
        if panel_index == 0:
            return
        
        # Check if panel already loaded
        if panel_index == 1 and self.parameter_filters_panel is None:
            logger.debug("Lazy loading: Range Filters panel")
            self.parameter_filters_panel = ParameterFiltersPanel(self.graph_index, self.all_signals, self)
            self.stacked_widget.replaceWidget(self.stacked_widget.widget(1), self.parameter_filters_panel)
            
        elif panel_index == 2 and self.static_limits_panel is None:
            logger.debug("Lazy loading: Static Limits panel")
            self.static_limits_panel = StaticLimitsPanel(self.all_signals, self)
            self.stacked_widget.replaceWidget(self.stacked_widget.widget(2), self.static_limits_panel)
            # Connect signals
        ...
        elif panel_index == 3 and self.basic_deviation_panel is None:
            logger.debug("Lazy loading: Basic Deviation panel")
            self.basic_deviation_panel = BasicDeviationPanel(self.all_signals, self)
            self.stacked_widget.replaceWidget(self.stacked_widget.widget(3), self.basic_deviation_panel)
            # Connect signals
        ...
        elif panel_index == 4 and self.advanced_deviation_panel is None:
            logger.debug("Lazy loading: Advanced Deviation panel")
            self.advanced_deviation_panel = self._create_placeholder_panel("Advanced Deviation", "📈")
            self.stacked_widget.replaceWidget(self.stacked_widget.widget(4), self.advanced_deviation_panel)
            
        elif panel_index == 5 and self.advanced_filters_panel is None:
            logger.debug("Lazy loading: Advanced Filters panel")
            self.advanced_filters_panel = self._create_placeholder_panel("Advanced Filters", "🔧")
            self.stacked_widget.replaceWidget(self.stacked_widget.widget(5), self.advanced_filters_panel)
    ...
    def _apply_settings(self):
        ...
        try:
            ...
            logger.debug(f"Applying Range Filters: {num_conditions} conditions")
            
            # Boş conditions bile emit et - bu filter temizlendi demektir
            self.range_filter_applied.emit(filter_conditions)
        except Exception as e:
            logger.error(f"Error applying range filters: {e}")
    ...
    def accept(self):
        ...
        logger.info("[DIALOG] OK button clicked, applying all settings before closing...")
        self._apply_settings()
        logger.info("[DIALOG] Closing dialog")
        super().accept()
