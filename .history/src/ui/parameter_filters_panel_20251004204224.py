...
class ParameterFiltersPanel(QWidget):
    ...
    def _add_range_condition(self):
        ...
        # PERFORMANCE: Lazy loading - initially only show first 100 items
        # Full list will load when user searches or scrolls
        param_list.setUpdatesEnabled(False)  # Disable updates during batch operation
        max_initial_items = 100
        signals_to_show = self.all_signals[:max_initial_items]
        
        # Show warning if truncated
        if len(self.all_signals) > max_initial_items:
            warning_item = QListWidgetItem(f"[Showing {max_initial_items} of {len(self.all_signals)} - use search to find more]")
            warning_item.setFlags(Qt.ItemIsEnabled)  # Not selectable
            warning_item.setForeground(Qt.yellow)
            param_list.addItem(warning_item)
