...
class ParametersPanel(QWidget):
    ...
    def __init__(self, all_signals: List[str], visible_signals: List[str] = None, parent=None):
        ...
        # PERFORMANCE: Lazy loading için
        self._items_loaded = False
        self._filter_text = ""
        self._max_visible_items = 100  # Bir seferde max gösterilecek item sayısı
    ...
    def _populate_signal_list(self):
        """Populate the signal list with checkable items. OPTIMIZED for large datasets with lazy loading."""
        # PERFORMANCE: Disable updates during batch operation
        self.signal_list.setUpdatesEnabled(False)
        self.signal_list.clear()
        
        # PERFORMANCE: Filter signals first
        if self._filter_text:
            filtered_signals = [s for s in self.all_signals if self._filter_text.lower() in s.lower()]
        else:
            filtered_signals = self.all_signals
        
        # PERFORMANCE: Limit visible items to prevent UI freeze with 300-400 signals
        signals_to_show = filtered_signals[:self._max_visible_items]
        
        # Show warning if truncated
        if len(filtered_signals) > self._max_visible_items:
            warning_item = QListWidgetItem(f"[Showing {self._max_visible_items} of {len(filtered_signals)} signals - use search to filter]")
            warning_item.setFlags(Qt.ItemIsEnabled)  # Not checkable
            warning_item.setForeground(Qt.yellow)
            self.signal_list.addItem(warning_item)
    ...
    def _setup_connections(self):
        ...
        # PERFORMANCE: Debounce search to avoid rebuilding list on every keystroke
        self.search_bar.textChanged.connect(self._on_search_text_changed)
    
    def _on_search_text_changed(self, text):
        """Handle search text change with debouncing."""
        self._filter_text = text
        # Immediately update the list (with lazy loading it's fast now)
        self._populate_signal_list()
        self._update_stats()
