...
from PyQt5.QtWidgets import (
    ...
    QDoubleSpinBox, QComboBox, QScrollArea, QFrame
)
...
class StaticLimitsPanel(QWidget):
    ...
    def _setup_ui(self):
        ...
        # Description
        desc = QLabel("Configure warning and error limits for signal values. Limits will be displayed as horizontal lines on the graph.")
        desc.setStyleSheet("""
            QLabel {
    ...
    def _create_signal_limit_widget(self, signal_name: str):
        ...
        warning_layout.addRow("Min Warning:", warning_min_sb)
        warning_layout.addRow("Max Warning:", warning_max_sb)
        
        # Error limits
        error_group = QGroupBox("🚨 Error Limits")
        error_group.setStyleSheet(self._get_subgroup_style())
        error_layout = QFormLayout(error_group)
        
        error_min_sb = QDoubleSpinBox()
        error_min_sb.setRange(-999999.0, 999999.0)
        error_min_sb.setDecimals(3)
        error_min_sb.setStyleSheet(self._get_spinbox_style())
        
        error_max_sb = QDoubleSpinBox()
        error_max_sb.setRange(-999999.0, 999999.0)
        error_max_sb.setDecimals(3)
        error_max_sb.setStyleSheet(self._get_spinbox_style())
        
        error_layout.addRow("Min Error:", error_min_sb)
        error_layout.addRow("Max Error:", error_max_sb)
        ...
        limit_config = {
            ...
            'error_min': error_min_sb,
            'error_max': error_max_sb
        }
        self.limit_configs[signal_name] = limit_config
        
        # Connect signals
        enable_cb.toggled.connect(lambda checked, name=signal_name: self._on_limit_changed(name))
        for widget in [warning_min_sb, warning_max_sb, error_min_sb, error_max_sb]:
