...
class StaticLimitsPanel(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.export_sample_btn = QPushButton("📋 Export Sample Excel")
        ...
        for i in range(self.limits_layout.count()):
            ...
            if item and item.widget():
                ...
                if hasattr(widget, 'title'):
                    ...
                    title = widget.title().replace('📊 ', '').strip()
    ...
    def _import_from_excel(self):
        ...
        try:
            ...
            for _, row in df.iterrows():
                ...
                warning_min = float(row['Warning_Min']) if pd.notna(row['Warning_Min']) else 0.0
                warning_max = float(row['Warning_Max']) if pd.notna(row['Warning_Max']) else 0.0
    ...
    def _export_sample_excel(self):
        ...
        try:
            ...
            # Try to import pandas for Excel writing
            try:
                import pandas as pd
            except ImportError:
                QMessageBox.warning(
            ...
            df.to_excel(file_path, index=False, sheet_name='Limits')
