...
class TimeGraphWidget(QWidget):
    ...
    def _on_graph_settings_requested(self, graph_index: int):
        ...
        # Use the new advanced settings dialog
        dialog = GraphAdvancedSettingsDialog(graph_index, all_signals, visible_signals, self)
    ...
    def _apply_range_filter(self, filter_data: dict):
        """Apply range filter to the specified graph."""
        logger.info(f"Applying range filter to graph {filter_data['graph_index']}: {filter_data}")
        
        try:
            # Get active tab and container
            active_tab_index = self.tab_widget.currentIndex()
            if active_tab_index < 0 or active_tab_index >= len(self.graph_containers):
                logger.warning("No active tab for filter application")
                return
            ...
            # Calculate time segments that satisfy all conditions
            time_segments = self._calculate_filter_segments(all_signals, conditions)
            
            if not time_segments:
                logger.warning("No time segments match the filter conditions")
                from PyQt5.QtWidgets import QMessageBox
                QMessageBox.warning(self, "No Matches", 
                                  "No time segments match the specified filter conditions.")
                return
                
            logger.info(f"Found {len(time_segments)} time segments matching filter conditions")
            
            # Apply filtering based on mode
            if mode == 'segmented':
                self._apply_segmented_filter(container, graph_index, time_segments)
            else:  # concatenated
                # Concatenated mode affects time axis, so we need to handle it specially
                self._apply_concatenated_filter_with_isolation(container, graph_index, time_segments)
    ...
    def _calculate_filter_segments(self, all_signals: dict, conditions: list) -> list:
        ...
        for condition in conditions:
            ...
            if param_name not in all_signals:
                logger.warning(f"Parameter {param_name} not found in signals")
                continue
            ...
            if len(y_data) != len(time_array):
                logger.warning(f"Data length mismatch for {param_name}")
                continue
        ...
        if in_segment:
            segments.append((segment_start, time_array[-1]))
    ...
    def _apply_segmented_filter(self, container, graph_index: int, time_segments: list):
        ...
        logger.info(f"Applying segmented filter to graph {graph_index} with {len(time_segments)} segments")
        ...
        for signal_name in visible_signals:
            ...
            for segment_start, segment_end in time_segments:
                ...
                if len(segment_x) > 0:
                    ...
                    plot_widget.plot(segment_x, segment_y, pen=color, name=f"{signal_name}_segment")
        ...
        QMessageBox.information(
            self,
            "Filter Applied",
            f"Segmented filter applied to Graph {graph_index + 1}.\n\n"
    ...
    def _apply_concatenated_filter_with_isolation(self, container, graph_index: int, time_segments: list):
        """Apply concatenated display filter with isolation to prevent affecting other graphs."""
        logger.info(f"Applying isolated concatenated filter to graph {graph_index} with {len(time_segments)} segments")
        
        # Show warning about concatenated mode
        from PyQt5.QtWidgets import QMessageBox
        reply = QMessageBox.question(
            self, 
            "Concatenated Display Mode", 
            f"Concatenated mode will modify the time axis for Graph {graph_index + 1}.\n\n"
            "This may affect the synchronization with other graphs in this tab.\n\n"
        ...
        if reply == QMessageBox.No:
            logger.info("Concatenated filter cancelled by user")
            return
            
        # Apply the concatenated filter
        self._apply_concatenated_filter(container, graph_index, time_segments)
        
        # Show info about the result
        QMessageBox.information(
            self,
            "Filter Applied",
            f"Concatenated filter applied to Graph {graph_index + 1}.\n\n"
            f"Time axis has been modified to show {len(time_segments)} segments continuously.\n\n"
            "Note: Other graphs in this tab may no longer be time-synchronized."
        )
