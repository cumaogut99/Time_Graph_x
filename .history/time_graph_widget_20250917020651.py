...
class TimeGraphWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.active_filters = {}  # tab_index -> filter_data
    ...
    def _redraw_all_signals(self):
        ...
        if self.filter_applied and self.active_filters:
    ...
    def _reapply_active_filters(self):
        ...
        try:
            logger.info(f"🔍 [FILTER DEBUG] Reapplying {len(self.active_filters)} active filters")
            for tab_index, filter_data in self.active_filters.items():
    ...
    def clear_active_filters(self):
        """Clear all active filters and redraw signals."""
    ...
    def _apply_range_filter(self, filter_data: dict):
        ...
        try:
            ...
            logger.info(f"🔍 [FILTER DEBUG] Calculated segments: {time_segments}")
            ...
            # Save filter state
            self.active_filters[active_tab_index] = filter_data.copy()
            self.filter_applied = True
            logger.info(f"🔍 [FILTER DEBUG] Saved filter state for tab {active_tab_index}")
    ...
    def _apply_segmented_filter(self, container, graph_index: int, time_segments: list):
        ...
        for signal_name in visible_signals:
            ...
            for i, (segment_start, segment_end) in enumerate(time_segments):
                logger.info(f"🔍 [SEGMENTED DEBUG] Processing segment {i+1}: {segment_start} to {segment_end}")
                ...
                logger.info(f"🔍 [SEGMENTED DEBUG] Segment {i+1} data points: {len(segment_x)}")
                
                if len(segment_x) > 0:
                    # Plot this segment
                    color = self._get_signal_color(signal_name)
                    logger.info(f"🔍 [SEGMENTED DEBUG] Plotting segment {i+1} with color {color}")
                    plot_widget.plot(segment_x, segment_y, pen=color, name=f"{signal_name}_segment_{i}")
                    segments_plotted += 1
    ...
    def _apply_global_concatenated_filter(self, container, time_segments: list):
        ...
        QMessageBox.information(
            ...
            f"Concatenated filter applied to all graphs in this tab.\n\n"
