...
class TimeGraphWidget(QWidget):
    ...
    def _apply_limit_lines_to_all_graphs(self):
        ...
        try:
            logger.info(f"[LIMITS DEBUG] Starting to apply limit lines to all graphs")
            active_tab_index = self.tab_widget.currentIndex()
            logger.info(f"[LIMITS DEBUG] Active tab index: {active_tab_index}")
            
            if active_tab_index < 0 or active_tab_index >= len(self.graph_containers):
                logger.info(f"[LIMITS DEBUG] Invalid tab index, returning")
                return
                
            container = self.graph_containers[active_tab_index]
            plot_widgets = container.plot_manager.get_plot_widgets()
            logger.info(f"[LIMITS DEBUG] Found {len(plot_widgets)} plot widgets")
            ...
            for graph_index, plot_widget in enumerate(plot_widgets):
                ...
                logger.info(f"[LIMITS DEBUG] Graph {graph_index} limits settings: {limits_settings}")
                ...
                if limits_settings and self.graph_renderer:
                    ...
                    logger.info(f"[LIMITS DEBUG] Graph {graph_index} visible signals: {visible_signals}")
                    
                    # Apply limit lines
                    self.graph_renderer._apply_limit_lines(plot_widget, graph_index, visible_signals)
                    logger.info(f"Applied limit lines to graph {graph_index} with {len(limits_settings)} limit configs")
                else:
                    logger.info(f"[LIMITS DEBUG] No limits settings or graph renderer for graph {graph_index}")
    ...
    def _on_graph_settings_requested(self, graph_index: int):
        ...
        dialog = GraphAdvancedSettingsDialog(graph_index, all_signals, visible_signals, saved_filter_data, saved_limits_data, self)
