...
class TimeGraphWidget(QWidget):
    ...
    def _on_graph_settings_requested(self, graph_index: int):
        ...
        dialog = GraphAdvancedSettingsDialog(graph_index, all_signals, visible_signals, saved_filter_data, saved_limits_data, None)
    ...
    def _on_basic_deviation_applied(self, graph_index: int, deviation_settings: Dict[str, Any]):
        ...
        logger.info(f"Applying basic deviation settings to graph {graph_index}: {deviation_settings}")
        
        try:
            # Apply deviation settings to graph renderer
            if hasattr(self, 'graph_renderer') and self.graph_renderer:
                self.graph_renderer.set_basic_deviation_settings(graph_index, deviation_settings)
            ...
            logger.error(f"Error applying basic deviation settings to graph {graph_index}: {e}")
