...
class TimeGraphWidget(QWidget):
    ...
    def _on_global_tooltips_changed(self, enabled: bool):
        ...
        # TODO: Implement tooltip functionality in plot widgets
    ...
    def _on_global_snap_changed(self, enabled: bool):
        ...
        # TODO: Implement snap to data functionality in plot widgets
    ...
    def _on_global_line_width_changed(self, width: int):
        ...
        active_container = self.get_active_graph_container()
        if active_container:
            # Apply line width to all signals in all graphs
            plot_widgets = active_container.get_plot_widgets()
            for graph_index, plot_widget in enumerate(plot_widgets):
                # Get all plot items (signals) in this graph
                for item in plot_widget.listDataItems():
                    if hasattr(item, 'setPen') and hasattr(item, 'opts'):
                        # Get current pen from opts
                        current_pen = item.opts.get('pen', None)
                        if current_pen is not None:
                            # Preserve the original color and style, only change width
                            if hasattr(current_pen, 'color') and hasattr(current_pen, 'style'):
                                new_pen = pg.mkPen(
                                    color=current_pen.color(), 
                                    width=width,
                                    style=current_pen.style()
                                )
                            elif hasattr(current_pen, 'color'):
                                new_pen = pg.mkPen(color=current_pen.color(), width=width)
                            else:
                                # Fallback: try to extract color from pen properties
                                try:
                                    # Get pen properties
                                    pen_color = current_pen.color() if hasattr(current_pen, 'color') else None
                                    if pen_color is None and hasattr(current_pen, 'brush'):
                                        pen_color = current_pen.brush().color()
                                    
                                    if pen_color:
                                        new_pen = pg.mkPen(color=pen_color, width=width)
                                    else:
                                        # Last resort: keep existing pen but try to change width
                                        new_pen = pg.mkPen(current_pen)
                                        new_pen.setWidth(width)
                                except:
                                    # If all else fails, create a basic pen with the width
                                    new_pen = pg.mkPen(width=width)
