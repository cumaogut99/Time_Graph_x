            ...
            for graph_index, plot_widget in enumerate(plot_widgets):
                ...
                for item in plot_widget.listDataItems():
                    if hasattr(item, 'setPen'):
                        try:
                            # Try to get the current pen from the curve
                            if hasattr(item, 'curve') and hasattr(item.curve, 'pen'):
                                current_pen = item.curve.pen()
                            elif hasattr(item, 'opts') and 'pen' in item.opts:
                                current_pen = item.opts['pen']
                            else:
                                # Skip if we can't get the pen
                                continue
                            
                            # Create new pen preserving color and style
                            if current_pen is not None:
                                # Get color from current pen
                            ...
                            logger.debug(f"Could not update line width for item: {e}")
                            continue
