...
class TimeGraphWidget(QWidget):
    ...
    def _on_global_tooltips_changed(self, enabled: bool):
        ...
        # Update plot manager with tooltip setting
        active_container = self.get_active_graph_container()
    ...
    def _add_tab(self, name: Optional[str] = None):
        ...
        # Create a new graph container for the tab
        graph_container = GraphContainer(self.theme_manager)
    ...
    def cleanup(self):
        ...
        try:
            # Example: Stop any running timers
            # if hasattr(self, 'my_timer') and self.my_timer.isActive():
            #     self.my_timer.stop()
    ...
    def get_layout_config(self):
        ...
        for i in range(self.tab_widget.count()):
            ...
            graph_containers = tab.findChildren(GraphContainer)
            
            graphs_config = []
            for container in graph_containers:
                signals = container.plot_manager.get_visible_signals()
                graphs_config.append({'signals': signals})
            
            tabs_config.append({'graphs': graphs_config})
        ...
        for i, tab_config in enumerate(config['tabs']):
            ...
            if 'graphs' in tab_config:
                ...
                if graph_count > 0:
                    active_container = self.get_active_graph_container()
                    if active_container:
                        active_container.plot_manager.set_graph_count(graph_count)
                        self._on_graph_count_changed(graph_count)

                    QApplication.processEvents()

                    for j, graph_config in enumerate(tab_config['graphs']):
                        container = self.graph_containers[j]
                        
                        if 'signals' in graph_config and self.data_manager.get_dataframe() is not None:
                            for signal_name in graph_config['signals']:
                                if signal_name in self.data_manager.get_dataframe().get_column_names():
                                    container.plot_manager.add_signal(signal_name)
