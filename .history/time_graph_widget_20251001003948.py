...
class TimeGraphWidget(QWidget):
    ...
    def _initialize_cursor_manager(self):
        ...
        if plot_widgets:
            ...
            if hasattr(self, 'graph_settings_panel_manager'):
                ...
                # Sync initial viewport lock setting from graph settings panel
                initial_viewport_lock = self.graph_settings_panel_manager.global_settings.get('cursor_viewport_lock', True)
                self.cursor_manager.set_viewport_lock(initial_viewport_lock)
    ...
    def _redraw_all_signals(self):
        ...
        # CRITICAL: Auto-range AFTER applying settings to show all data
        # This must come after _apply_saved_graph_settings() to override any saved view ranges
        for tab_index, container in enumerate(self.graph_containers):
            ...
            for plot_widget in plot_widgets:
                ...
                # Then trigger auto-range to fit all data
        ...
        logger.debug("Auto-ranged all plots (X and Y axes) after signal redraw and settings apply")
    ...
    def _on_cursor_viewport_lock_changed(self, enabled: bool):
        """Handle cursor viewport lock toggle."""
        logger.info(f"Cursor viewport lock {'enabled' if enabled else 'disabled'}")
        
        # Update cursor manager with viewport lock setting
        if hasattr(self, 'cursor_manager') and self.cursor_manager:
            self.cursor_manager.set_viewport_lock(enabled)
    ...
    def _setup_connections(self):
        ...
        self.graph_settings_panel_manager.cursor_viewport_lock_changed.connect(self._on_cursor_viewport_lock_changed)
