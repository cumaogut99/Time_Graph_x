    ...
    def _on_per_graph_view_reset(self, graph_index: int):
        ...
        if active_container:
            ...
            if 0 <= graph_index < len(plot_widgets):
                ...
                # because they share the same X range. We'll do a global X auto-range first.
                if len(plot_widgets) > 0:
                    # Step 1: Enable X auto-range for ALL plots (they're linked)
                    for pw in plot_widgets:
                        pw.enableAutoRange(axis='x', enable=True)
                    
                    # Step 2: Trigger auto-range on the first plot to set X range for all
                    plot_widgets[0].autoRange(padding=0)
                    
                    # Step 3: Disable X auto-range for all plots
                    for pw in plot_widgets:
                        pw.enableAutoRange(axis='x', enable=False)
                    
                    logger.debug(f"[VIEW RESET] X-axis auto-ranged globally for all {len(plot_widgets)} linked plots")
