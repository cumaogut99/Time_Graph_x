...
class TimeGraphWidget(QWidget):
    ...
    def _initialize_cursor_manager(self):
        ...
        if plot_widgets:
            ...
            # Always enable viewport lock for cursors (default behavior)
            self.cursor_manager.set_viewport_lock(True)
    ...
    def _redraw_all_signals(self):
        ...
        for tab_index, container in enumerate(self.graph_containers):
            # Block cursor viewport updates during auto-range
            if hasattr(container, 'cursor_manager') and container.cursor_manager:
                container.cursor_manager.block_view_range_signals(True)
            ...
            # Reset cursors BEFORE unblocking signals to avoid triggering view range changes
            if hasattr(container, 'cursor_manager') and container.cursor_manager:
                container.cursor_manager.reset_cursors_to_viewport()
                # Unblock signals AFTER cursor reset is complete
                container.cursor_manager.block_view_range_signals(False)
    ...
    def _on_per_graph_view_reset(self, graph_index: int):
        ...
        active_container = self.get_active_graph_container()
        if active_container:
            # Block cursor updates during this specific auto-range
            if hasattr(active_container, 'cursor_manager') and active_container.cursor_manager:
                active_container.cursor_manager.block_view_range_signals(True)

            plot_widgets = active_container.get_plot_widgets()
            if 0 <= graph_index < len(plot_widgets):
                plot_widget = plot_widgets[graph_index]
                
                # CRITICAL FIX: For X-axis, we need to consider ALL linked plots
                # because they share the same X range. Calculate min/max across ALL plots.
                if len(plot_widgets) > 0:
                    # Find the global X range across all plots
                    x_min = float('inf')
                    x_max = float('-inf')
                    
                    for pw in plot_widgets:
                        # Get all items in this plot
                    ...
                    # If we found valid data, set the X range for all plots
                    if x_min != float('inf') and x_max != float('-inf'):
                        for pw in plot_widgets:
                            pw.setXRange(x_min, x_max, padding=0.02)
                        logger.info(f"[VIEW RESET] X-axis set to [{x_min:.2f}, {x_max:.2f}] for all {len(plot_widgets)} linked plots")
                    else:
                        logger.warning("[VIEW RESET] No valid X data found in any plot")
                    
                    logger.info(f"[VIEW RESET] X-axis auto-ranged globally for all {len(plot_widgets)} linked plots")
                
                # Step 4: Now do Y-axis auto-range for the specific plot
                plot_widget.enableAutoRange(axis='y', enable=True)
                plot_widget.autoRange()
                plot_widget.enableAutoRange(axis='y', enable=False)
                
                logger.info(f"View reset for graph {graph_index} - showing all data including limit lines")
            
            # Reset cursors BEFORE unblocking signals
            if hasattr(active_container, 'cursor_manager') and active_container.cursor_manager:
                active_container.cursor_manager.reset_cursors_to_viewport()
                # Unblock signals AFTER cursor reset
                active_container.cursor_manager.block_view_range_signals(False)
