    ...
    def _on_per_graph_view_reset(self, graph_index: int):
        ...
        if active_container:
            ...
            if 0 <= graph_index < len(plot_widgets):
                # Simple solution: Just call autoRange() on each plot
                # PyQtGraph handles linked axes automatically
                for plot_widget in plot_widgets:
                    plot_widget.autoRange()
                logger.info(f"View reset for graph {graph_index} - all {len(plot_widgets)} plots auto-ranged")
            
            # Reset cursors AFTER auto-range and BEFORE unblocking signals
            if hasattr(active_container, 'cursor_manager') and active_container.cursor_manager:
