...
class TimeGraphWidget(QWidget):
    ...
    def _on_graph_settings_requested(self, graph_index: int):
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            logger.info(f"[DIALOG] Updated signals for Tab {target_tab_index}, Graph {graph_index}: {selected_signals}")
    ...
    def _apply_range_filter(self, filter_data: dict):
        ...
        try:
            # CRITICAL FIX: Use target tab from dialog, NOT current active tab!
            # Dialog açıldığı tab'a uygulanmalı, şu anda hangi tab aktifse değil!
            target_tab_index = getattr(self, '_dialog_target_tab', self.tab_widget.currentIndex())
            
            if target_tab_index < 0 or target_tab_index >= len(self.graph_containers):
                logger.error(f"Invalid target_tab_index: {target_tab_index}")
                return
            
            logger.info(f"Applying filter to target_tab_index: {target_tab_index} (dialog opened for this tab)")
            ...
            # CRITICAL FIX: graph_index filter_data'dan gelmeli AMA
            # eğer active_tab_index ile uyuşmuyorsa BU BİR HATADA!
            # Dialog açıldığında hangi grafik için açıldıysa o graph_index kullanılmalı
